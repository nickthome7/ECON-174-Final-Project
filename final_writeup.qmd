...
To try and copy the methodologies used behind the LSTM model and its success, a rolling window was implemented for the ARIMA model itself. Utilizing a 90 minute rolling window until the data no longer has values to insert into that rolling window. We expected the model to perform better, however, we ended up getting the same results of the model hovering around the average values and using the same random walk method to change the values at the 0.001 decimal place. Which is not extraordinarily useful for stock prediction methods. We created the visualizations based on block groups, i.e. the first block group of the test set is going to be the minutes 1-90, then the second block group is going to be 91-180. These graphs show drastic up and downs for the true values repeated roughly 100 fold times. The rolling window for NVDA did not assist in a stronger prediction value but rather hovered near the average as previously described. This is one limitation of using an ARIMA model on data such as this, as it has no true deviation to base itself on and rather changes at a very small gradient level. However, for NVDA in particular, the one shot ARIMA model did stay nearly directly in the middle of the test set and did surprisingly well capturing the average of the first 90 minutes of the test set.
...
Here we utilize the previously made LSTM files in the ".h5" format, and use the test sets to predict the next 10 time points for each NVDA and META. As the values in the time points increase, i.e. current time + 10, the accuracy does drop, as it is tougher to predict future time points the further in the future in comparison to time points closer to relative time. We do see stronger accuracies for the NVDA model due to the NVDA data having more time points. This is due to NVDA being traded more frequently than META and the price updating more often, allowing for more one-minute bars of testing and training data for NVDA in comparison to META. While the predictions are not perfect, they are significantly more accurate than the ARIMA model, with accuracies within the 99.9% range of the true data when looking at the next minute, i.e. current time + 1.
...
Now we get to the point of being able to test the different prediction models and their applications to a simulated trading experience. We will use a DQN model that uses the following mathematical concepts:
...
Between these mathematical concepts of the DQN model, the model will update trading behavior dependent on "r" which is the reward function within the model. Such that is updated in our code as "current portfolio - previous portfolio", taking the differences between the two equity amounts at time t, will allow the DQN model to learn what a positive and negative reward is like. Therefore, if the reward is expected to be negative then the model will learn to not make that trade, and if the expected reward is positive then the model will make that trade. We allow the model to start with a \$100,000 starting fund, and each trade is traded in quantities of 5 shares per trade. The equity is updated at each step and the model will not make trades if there is no cash in the account, which does not happen as each trade is cleared before the next trade is made. Throughout the trading period, using the test data, when we plot the equity curves, we can observe that no trades are made for the ARIMA model. This is due to the ARIMA model holding that constant horizontal prediction value around the average value of the training data. While the LSTM model can dynamically adjust to the price fluctuations, we see that trades are made and the model makes profitable trades dependent on the LSTM model's predictions.
...
In conclusion, we can come to the consensus that the ARIMA model was not a beneficial model for these trading parameters. Whether the model could be optimized or not, we believe that the ARIMA model will not be able to adjust to the short term predictions to the same degree of accuracy as the LSTM model. While the ARIMA model has its drawbacks in this case, as we want to be able to predict short intervals with high accuracy, the LSTM model has these drawbacks in long term accuracies. Whereas the further out an LSTM model is used to predict data the less accurate it is, since seasonality does not come into account in an LSTM model, an ARIMA model can be extremely beneficial for predicting seasonality based data such as we covered in class. However, as mentioned, in this case the LSTM model is significantly stronger for short term predictions with high accuracies. Therefore, allowing the LSTM model to be the dominant price prediction model in our tests between ARIMA and LSTM for the minute-bar level data for NVDA and META.
